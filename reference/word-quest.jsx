...
const SENTENCES = {
 ...
 "43": [{"id":"s:v3-l43-01","text":"We packed the backpack. Mom asked, milk? I picked milk. Dad helped us zip it up."},{"id":"s:v3-l43-02","text":"I got up and dressed. Mom asked me to help, so I picked up the pots and brushed the mat. Dad mixed a big pot of jam. When it was set, we had it on the step in the sun."}],
 "44": [{"id":"s:v3-l44-01","text":"I filled the tub with mom. I got in with my duck. Then I did a splash. I yelled to mom, and she got wet! We had fun."},{"id":"s:v3-l44-02","text":"Dad filled the big tub with sand and set it in the grass. I filled my cup with sand and did tip it up into a hill. Then my brother ran up and did kick it! I yelled, and he sat and helped me fix it. We filled the tub up, and my hill was as big as the tub."}],
...
/* v6, the 2026-08-20 cutover: the 21-level world became the 100-level
   ...
   and the pre-ladder recovery both use. MEASURED LIMIT (2026-08-20 audit):
   that recompute walks no higher than level 6 even for a reader with all 476
   old words at box 5, because new level 6 seats cops and spots and the old
   bank taught neither - so for any save stored at level 6 or above the floor
   wins and this reduces to s.level = min(stored, 100). The stored NUMBER now
   addresses a re-numbered ladder, so an updating child skips whatever new
   code sits below their old number; docs/open-faults.md carries this as an
   open fault awaiting the owner's seating table. A save with no graded words
   keeps its number and nothing else; the identity floor is deliberately
   modest, and the owner may re-rule it with a table any day. */
function migrateV6(s) {
  ...
  s.level = Math.max(Math.min(s.level || 1, LEVELS.length), lvl);
  s.version = 6;
}
function migrate(s) {
  s = heal(s);
