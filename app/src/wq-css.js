...
/* THE GLOWSEED (art step 2, bible 7; the council's before pass, 2026-08-23).
   ...
   ovoid, the core offset toward the cradle end (bottom) AND off the
   vertical axis, with one asymmetric tip at the top right, so it is never an
   eye, an LED or a bullseye (17.1's face risk). The first draft was four
   concentric ovals with the core dead-centre in x and 2 px low in y: the art
   director failed checkpoint 1 on it and both verifiers measured the same
   thing - the caps 6 px each at 1x, the core's x-centre exactly the box's -
   so the shape is asymmetric in both axes now and the silhouette is recorded
   in the provenance lock, since order D says the step-6 seed keeps it.
   A 1 px rim, thinner than the tile's 3 px ring. Idle:
   ...
   core gone and the rim DASHED - a shape, since a fill alone measured
   1.25:1 against the sky and vanished in greyscale (the after pass). No transition, no animation at any phase: the
...
/* THE WORD PAINTS ABOVE THE TILE ROW (art step 1, the fifth and sixth
   judgements, 2026-08-22). The row is later in tree order and isolates its
   stacking, so a sounding tile's band and ring painted OVER the word's
...
/* The open word is marked by the SAME ring the sound-out tiles take, so the
   two never say different things about "this one, now" - in the same colour
   since art step 1 (2026-08-22): cyanStructural, 4.73, 4.61 and 5.05:1 on the
   gradient's three stops, where the action red measured 2.95, 2.88 and 3.15
   (open-faults AB, closed by this step). */
.wq-sword-open{outline:3px solid ${C.cyanStructural};outline-offset:-3px}
...
.wq-slot-tiles.wq-many .wq-tile{padding:4px 8px;font-size:clamp(.85rem,2.4svh,1.15rem);border-radius:9px;--wqband:7px}
.wq-slot-tiles.wq-many{gap:4px}
.wq-slot-tiles.wq-crowd .wq-tile{padding:3px 5px;font-size:clamp(.7rem,2svh,.95rem);border-radius:7px;--wqband:5px}
.wq-slot-tiles.wq-crowd{gap:3px}
/* The sounding state (bible 11): a 3 px structural cyan ring at offset 0,
   ...
   ring plus band exceeds the gap. The ring
...
@media (max-height:520px){
  ...
  .wq-tile{padding:2px 7px;border-radius:8px;font-size:clamp(.85rem,4svh,1.1rem);--wqband:7px}   /* ring 3 + band 4 across the short stage's 4 px gap */
