# Batch 21: the 143 words the 100-level ladder needs and the pack does not have.
#
# WHY THESE 143. Every one is on the owner's own 420-word target list, so its
# membership does not depend on the ladder rebuild now under way - a rebuild can
# move a word between levels but cannot remove a word the owner ruled in. That
# is what makes it safe to record them before the shape is final.
#
# CHECKED FIRST, as E10 requires: all 143 were run through round_guard.history()
# against tools/voice-words.csv and the waiting room ledger. None has been
# through a round. Every field here is a genuine first offer.
#
# FAMILIES, in the order the rounds have earned. Batch 19's measured result:
# 22 of 23 accepts were carrier cuts, and "Say {word}, everybody." won 12 of
# them. So Say leads, then listen, here, word. No raw solo is offered - af_heart
# puts an 85-115 ms voiced blob at the start of every isolated render, and a
# solo arm is the blob plus the word. Solos go through clean_onset first.
# A thin field is answered with MORE CARRIER SHAPES, never with an ungated
# fallback: a thin field is a real answer, a padded one is a wasted evening.
#
# Usage: python3 tools/render_batch21.py <out_dir> [start] [count]
import base64
import hashlib
import json
import pathlib
...
# EVERY word the owner has ruled onto the target list that has no clip yet.
# Not the ladder draft: the draft is being rebuilt, and a bill measured against
# a draft goes stale the moment the draft moves - which is exactly what happened
# to the "143" on 2026-08-18. The target list is the owner's ruling and is stable.
_man = _json.loads((REPO / "app/public/voice/manifest.json").read_text(encoding="utf-8"))
_pend = _json.loads((REPO / "tools/pending-words/pending-words.json").read_text(encoding="utf-8"))
_have = {k[2:] for k in _man if k.startswith("w:")} | {k for k in _pend if not k.startswith("_")}
_tgt = [l.strip() for l in (REPO / "tools/target-vocab.txt").read_text(encoding="utf-8").splitlines()
        if l.strip() and not l.startswith("#")]
_all = sorted(set(_tgt) - _have)
_start = int(sys.argv[2]) if len(sys.argv) > 2 else 0
_count = int(sys.argv[3]) if len(sys.argv) > 3 else len(_all)
WORDS = _all[_start:_start + _count]
print("batch 21: %d words to render, %d of %d needing a clip" % (len(WORDS), _start, len(_all)), flush=True)
...
def build():
    """Writes after EVERY word, so a round can be served from a partial run.
    The owner asked for batches as they land; a job that only writes at the end
    hands him nothing for two hours and then everything at once."""
    out = {}
    part = OUT / "batch21-audio.json"
    for word in WORDS:
        ...
        print(f"  {word}: {len(out[word])} arms  ({len(out)}/{len(WORDS)})", flush=True)
    return out


if __name__ == "__main__":
    result = build()
    (OUT / "batch21-audio.json").write_text(json.dumps(result), encoding="utf-8")
    thin = [w for w, a in result.items() if len(a) < 3]
    print(f"wrote batch21-audio.json; {sum(len(a) for a in result.values())} arms over {len(result)} words"
