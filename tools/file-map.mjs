...
export const DECLARED = [
  ...
  { path: "tools/s9-art-sources.json", kind: "DATA", owns: "S9 fifth exception (owner-ruled 2026-09-04): the public-domain photograph URLs the season colours were measured from, scoped to provenance.json by exact URL; twelve carry a personal name inside them and pass nowhere else" },
  ...
  { path: "tools/art-lay.py", kind: "GATE", owns: "the lay gate (bible 4.1, owner-ruled 2026-09-04): a two-ink texture is a dither only if it has no direction - run-length ratio under 2.0 in both axes; a lay, a stitch or a course passes. Not yet in check: waits for the composed scene." },
  { path: "tools/art-frame-contrast.py", kind: "GATE", owns: "the frame gate (bible 8.3 on the RENDER, owner-ruled 2026-09-04): no adjacent pair anywhere in the composed frame over the word's own ratio, and no sprite pixel within 3 px of the reading field. Not yet in check: today's compose() fails it by 2160 pairs and the scene that replaces it is being composed." },
  { path: "tools/art-repeats.json", kind: "DATA", owns: "the repeats gate's ledger (tools/art-repeats.py): every block of pixels a sprite is allowed to contain twice, keyed on its measurement so a redraw loses the exemption, and with the reason a person wrote. Empty since 2026-09-03, when all sixteen were cleared." },
  ...
  { path: "tools/pending-chunks/ledger.jsonl", kind: "LEDGER", owns: "the chunk waiting room: verdict, family, carrier, phonemes and byte pin for every approved-and-unshipped chunk clip from the CHUNK rounds (JSONL - the owner's stated lean, 2026-08-29); its bytes sit beside it as mp3 files, and each refused round's verdicts-with-comments sit beside it verbatim (round2-verdicts.txt); docs/voice-pack.md tells each round's story; merges into the word waiting room's machinery when the clips ship" },
  { path: "tools/pending-chunks/ledger.jsonl", kind: "LEDGER", owns: "the chunk waiting room: verdict, family, carrier, phonemes and byte pin for every approved-and-unshipped CHUNK clip from the chunk rounds (JSONL - the owner's stated lean, 2026-08-29); its bytes sit beside it as mp3s, and it merges into the one waiting room when the chunks ship" },
  ...
  { path: "tools/ladder/word-bill.json", kind: "DATA", owns: "the candidate words offered for the starved levels, and the owner's ruling on them" },
  { path: "tools/ladder/corpus-harvest.json", kind: "DATA", owns: "what the public-domain pipeline found: sentences and contiguous passages with their credits and source indices" },
  { path: "tools/corpus/sources.json", kind: "DATA", owns: "the public-domain books a ledger credit may name, each pinned by sha256 (G26 rule 5), with the year and evidence that puts each one in the public domain" },
  { path: "tools/corpus/excisions.json", kind: "DATA", owns: "every sentence the corpus screen removed from a book, verbatim, with its index and hazard class — the record that makes a screened text honest rather than a quietly edited one" },
  { path: "tools/target-vocab.txt", kind: "DATA", owns: "the 697-word target vocabulary the owner ruled the game covers before it is called done — SPEC section 12 owns the ruling" },
...
export const BULK = [
  ...
  { glob: /^tools\/corpus\/.*\.txt$/, kind: "DATA", note: "public-domain source text, read by G26 rule 5 and pinned in sources.json" },
