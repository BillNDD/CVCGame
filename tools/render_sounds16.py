...
REF = {
    ...
    "w": [(3.97, 4.32, [591, 1070, 2947]), (7.63, 7.96, [621, 948, 3161])],
    "z": [(5.51, 5.90, [792, 849, 1475]), (6.00, 6.16, [627, 2047, 2281])],
    "h": [(13.65, 13.79, [1062, 1586, 2515]), (17.23, 17.44, [1105, 1562, 2484])],
}
SOURCE = {"v": ["van", "vet", "vex"], "w": ["web", "win", "wag", "wet"],
          "z": ["zip", "zap", "zig", "zag"], "h": ["hat", "hum", "hen", "hop"]}
            ...
            for w in SOURCE[name]:
                ...
                for hold in (140, 200, 260):
                    ...
                    if not src or len(src) < 3:
                        continue
                    for amt, tag in ((1.0, "full"), (0.7, "most")):
                        out = warp_to(seg, psr, src, tgt, amt)
                        if out is None:
                            continue
                        if name == "h":
                            out = tilt(devoice(out, psr), psr, 1000, -6.0)
