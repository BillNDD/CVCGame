...
def main():
    batch = json.loads((OUT.parent / "sbatch15-render.json").read_text(encoding="utf-8"))
    ...
    import io as _io2
    _io2.open(OUT / "sbatch15-audio.json", "w", encoding="utf-8", newline="\n").write(json.dumps(out))
    print("wrote sbatch15-audio.json; %d takes" % len(out))
