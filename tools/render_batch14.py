# Batch 14: lids and be, with options that are actually new.
#
# WHY THIS ROUND EXISTS. Batch 13 was sent as "a wide field" for lids. All six
# of its lids arms were byte-identical to batch 12's, and the arm the owner
# marked perfect (lids_2) WAS the arm they had marked closest one round earlier
# (lids_1). be_1 was likewise be_7, already marked "closest, weird trilling at
# end of e". The cause: batch 13's new mechanisms went into the two-letter code
...
# it is a check, and it prints what it dropped. A round that cannot fill a
# field with NEW material must say so rather than pad itself with old material
# wearing a new label.
#
# WHAT IS ACTUALLY NEW HERE:
#   - The closure frames reach the WORD path. "Sit. Lids. Sit." and
#     "Tuck. Lids. Tuck." put a full word mid-phrase between real neighbours
#     with measured silence on both flanks - the treatment that produced the
#     two-letter winners, never yet applied to a word.
#   - Speeds no earlier round used for these words: 0.65, 0.7, 1.05.
#   - Tail trims for lids. It ends in /z/, which can buzz on after the word is
#     over, and no round has offered lids without its tail.
#   - Warmth settings not yet tried: a brighter f0 (1.03) and a slightly
#     narrowed formant (0.97), against the darker pair used so far.
#   - be gets four more stop-final frames and a deeper set of tail trims,
#     because the e-tail is its named fault and batch 13's trims were shallow.
#
# Usage: python render_batch14.py <out_dir>
import base64
import hashlib
import json
import pathlib
...
RETRY = {
    "lids": "every option here is new — nothing from batch 12 or 13 can appear",
}
TWO_LETTER = ["be"]
COMPOUND = {}

LISTEN_SPEEDS = (0.65, 0.7, 1.05)   # speeds no earlier round used for these words
OTHER = [("everybody", "{W}, everybody."), ("say", "Say {w}, everybody.")]

items, failures = [], []

# THE HASH GUARD. Every arm hash the owner has already been offered, from every
# earlier round whose data is still on disk. An arm matching one of these is
# not a candidate: it is a clip they have already judged, and offering it again
# under a fresh label is what made batch 13 worthless for lids.
...
print(f"hash guard: {len(ALREADY)} arms the owner has already been offered, "
      f"across {len(list(ROUNDS.glob('out-*/batch-data.json')))} rounds")
repeats = []
...
def build_word(word, note, compound=False):
    ...
    # CLOSURE FRAMES REACH THE WORD PATH. A full word set mid-phrase between
    # real neighbours, with measured silence on both flanks - the treatment
    # that produced every two-letter winner, never yet applied to a word.
    # Located by the carrier's own silence, so no template match is involved.
    W = word.capitalize()
    for fam, frame in (("sit", f"Sit. {W}. Sit."), ("tuck", f"Tuck. {W}. Tuck."),
                       ("stop", f"Stop. {W}. Stop."), ("pack", f"Pack. {W}. Pack.")):
        for sp in (0.6, 0.7, 0.8):
            car, csr = say(frame, sp)
            for st, en, pre, post, idx in speech_islands(car, csr):
                if idx == 0 or pre < 60 or post < 60:
                    continue
                offer(f"{fam}_sp{sp}", car[st:en], csr, recover=False)
    ...
    # TAIL TRIMS. lids ends in /z/, which can buzz on after the word is over,
    # and no round has offered it without its tail. The gate still judges every
    # trim: one that takes too much fails the clipped check on its own.
    for fam, cut, csr, d in list(cands)[:6]:
        for ms in (60, 100, 140):
            ...
            offer(f"{fam}_cut{ms}", t, csr, recover=False)

    # warmth on the best listen cuts only — the family that wins.
    # A brighter f0 and a narrowed formant join the darker pair used so far.
    ...
    for fam, cut, csr, d in best:
        for tag, kw in (("warm", dict(f0r=0.97, fmt=1.03)), ("breath", dict(breath=1.25)),
                        ("bright", dict(f0r=1.03, fmt=0.97)), ("low", dict(f0r=0.94))):
            try:
...
def build_two_letter(word):
    ...
    FRAMES = (("rep", f"{W}. {W}. {W}."), ("sit", f"Sit. {W}. Sit."),
              ...
              ("kick", f"Kick. {W}. Kick."), ("cup", f"Cup. {W}. Cup."),
              ("quick", f"{W} quick. {W} quick."), ("top", f"Top. {W}. Top."))
    for sp in (0.6, 0.65, 0.7, 0.8, 0.85):
...
for word, note, compound in PLAN:
    ...
    for fam, cut, csr, d in cands:
        ...
        if sha in ALREADY:
            repeats.append((word, fam, ALREADY[sha]))
            failures.append((word, fam, f"already offered as {ALREADY[sha]}")); continue
...
(OUT / "batch-data.json").write_text(json.dumps({
    "title": "Batch 14 — lids and be, every option new and hash-checked",
