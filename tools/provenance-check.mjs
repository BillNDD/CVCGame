...
export function lockFromSources(css = readFileSync("app/src/wq-css.js", "utf8"), screen = readFileSync("app/src/screens/BuildItScreen.jsx", "utf8")) {
  ...
  return {
    ...
    extraPerLetter: num(screen, /small \? \d+ : (\d+)\)/),
    compact: { below: num(screen, /const COMPACT_BELOW = (\d+)/), box: num(screen, /small \? (\d+) : SLOT\)/), step: num(screen, /small \? (\d+) : 26\)/), gap: num(screen, /small \? (\d+) : 10\)/) },
