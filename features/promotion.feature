Feature: Level promotion
  ...
  # (">=" to ">") is what it protects against. Level 2 holds exactly ten words
  # in the converted ladder (its heart, i, is seated inline and counts like
  # any other), so its ten are the whole denominator.
  Scenario: Exactly 80 percent promotes
    Given a player on Level 2 with 8 of the 10 words at box 3
    When the session ends
    Then the player is promoted to Level 3

  Scenario: Just under 80 percent does not promote
    Given a player on Level 2 with 7 of the 10 words at box 3
    When the session ends
    Then the player stays on Level 2

  Scenario: Box 2 words are not solid
    Given a player on Level 1 with 10 of the 10 words at box 2
    When the session ends
    Then the player stays on Level 1

  # The converted Level 1 holds exactly ten decodables and no hearts (the
  # first heart, "the", seats at level 3), so the arithmetic is the level's
  # own: 80 per cent of 10 is 8, so 8 promotes and 7 does not.
  # Re-derived at the 2026-08-20 cutover; the old line remembered hearts
  # Level 1 no longer carries.
  Scenario: The starter level needs 8 of its 10 words
    Given a player on Level 1 with 8 of the 10 words at box 3
    When the session ends
    Then the player is promoted to Level 2

  Scenario: Seven of the starter level's 10 words is not enough
    Given a player on Level 1 with 7 of the 10 words at box 3
    When the session ends
    Then the player stays on Level 1

  Scenario: The last level has no promotion
    Given a player on Level 100 with 12 of the 12 words at box 5
    When the session ends
    Then the player stays on Level 100

  Scenario: Two perfect sessions in a row promote
    Given a player on Level 2 with 5 of the 10 words at box 3
