Feature: Level promotion
  ...
  Scenario: Exactly 80 percent promotes
    Given a player on Level 4 with 32 of the 40 words at box 3
    When the session ends
    Then the player is promoted to Level 5

  Scenario: Just under 80 percent does not promote
    Given a player on Level 4 with 31 of the 40 words at box 3
    When the session ends
    Then the player stays on Level 4
  ...
  Scenario: The last level has no promotion
    Given a player on Level 7 with 53 of the 53 words at box 5
    When the session ends
