...
| `tools/pending-chunks/ledger.jsonl` | LEDGER | the chunk waiting room: verdict, family, carrier, phonemes and byte pin for every approved-and-unshipped chunk clip from the CHUNK rounds (JSONL - the owner's stated lean, 2026-08-29); its bytes sit beside it as mp3 files; docs/voice-pack.md tells each round's story; merges into the word waiting room's machinery when the clips ship |
