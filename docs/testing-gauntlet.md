...
| `g31_dup_regions_max` | 7 | merged duplicated regions (14 at birth; 12 after the owner's 2026-09-13 correction, see below; 11 on 2026-09-15 when the runners' shared mechanics became tools/lib/runner.mjs; 10 after the pristine-control report mechanics joined it; 8 after the lock and anchor mechanics moved; 7 on 2026-09-16 when the two mutant gates' outcome report joined tools/lib/runner.mjs) |
...
- Assert measurements and numbers. Never assert a screenshot. Key: `g7_interface_checks` (70) on Chromium and `g7_interface_checks_webkit` (69), each its own gauntlet step. Three of those checks were added on 2026-09-02, when a release gauntlet went red on WebKit under load: the toast is placed against the measured height of the rail and the strip, and nothing re-measured when a zone grew without a render - a wrapped strip line lifted the child's controls into a toast that had not moved. The control grows the strip after the toast is up and reads the gap. **G7 does not yet run on Firefox** (open fault AY): it reached 23 green checks there and then lost a grade twice in three runs and twice reached the reveal's 10 s backstop, none of it pinned - and a gate red for unpinned reasons is a flaky gate. G8 and G18 run on Firefox; G7 joins when both faults are understood.
...
- Three of those 23 are B18's, and each was watched RED before the fix went in: a fully
  ...
  player that never wakes still hands over to system speech and still names `suspended`
  (red against a bare await, which would trade a missing ring for ten seconds of
  silence); and an utterance silenced during that wait schedules nothing (red against a
  fix with no second token check, which let a stale reveal play over a newer one).
...
- Baseline floor: `g17_governing_files` (51). It moved from 23 on 2026-08-11, when the
...
- **HISTORY is a ceiling, not a floor.** The only declaration that legitimises a file with
  ...
  read as live.
- Keys: `g23_declared` (66), `g23_facts` (4), `g23_controls` (40), problems capped at 0,
